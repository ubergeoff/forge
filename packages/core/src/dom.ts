...
/**
 * A function that renders slot content into a given ComponentContext.
 * Slot functions are compiled from the children passed to a component element
 * by its parent template, and called by the host component at each `<slot>`.
 */
export type SlotFn = (ctx: ComponentContext) => Node;
 ...
 * tracking, invokes the factory with the given props and slots, and returns
 * the root DOM node so the caller can insert it into the tree.
 *
 * Props are always passed as getter functions so both static and reactive
 * values share a uniform call-site API: `props['label']()`.
 *
 * Slots are functions `(ctx) => Node` compiled from the children of the
 * component element in the parent's template. The host component calls them
 * via `renderSlot()` at each `<slot>` position.
 *
 * @example
...
export function mountChild(
  factory: (ctx: ComponentContext, props: Record<string, () => unknown>, slots: Record<string, SlotFn>) => Node,
  parentCtx: ComponentContext,
  props: Record<string, () => unknown> = {},
  slots: Record<string, SlotFn> = {},
): Node {
  ...
  const node = factory(childCtx, props, slots);
  ...
  if (hmrId) {
    ...
    if (hmr?.instances) {
      ...
      list.push({ node, ctx: childCtx, props, slots, parentCtx });
...
/**
 * Renders named slot content at a `<slot>` position inside a host component.
 * Called by compiled component templates — not intended for direct use.
 *
 * Returns the node produced by the slot function, or an empty comment
 * placeholder when the caller provided no content for that slot.
 *
 * @example
 * // Generated by: <slot name="header">
 * const _e2 = renderSlot(slots, 'header', ctx);
 * insert(_wrapper, _e2);
 */
export function renderSlot(
  ...
  const fn = slots[name];
  return fn ? fn(ctx) : document.createComment('');
}
...
export function hmrAccept(
  id: string,
  newFactory: (ctx: ComponentContext, props: Record<string, () => unknown>, slots: Record<string, SlotFn>) => Node,
): void {
  const w = typeof window !== 'undefined' ? (window as unknown as Record<string, unknown>) : null;
  const hmr = w?.['__vorra_hmr'] as { instances?: Map<string, HMRInstance[]> } | undefined;
  ...
  for (const inst of instances) {
    ...
    const newNode = newFactory(newCtx, inst.props, inst.slots);
