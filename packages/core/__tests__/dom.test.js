...
import { createElement, setAttr, setProp, listen, insert, remove, bindText, bindAttr, bindProp, bindShow, bindClass, createComponent, destroyComponent, mountComponent, } from '../src/dom.js';
...
describe('bindClass', () => {
    ...
    it('handles classes being removed from the record', () => {
        ...
        const el = createElement('div');
        const handle = bindClass(el, () => showExtra() ? { base: true, extra: true } : { base: true });
...
describe('destroyComponent', () => {
    ...
    it('recursively destroys child contexts', () => {
        ...
        const count = signal(0);
        let childRuns = 0;
        child.effects.push(bindText(document.createTextNode(''), () => {
            count();
            childRuns++;
            return '';
        }));
