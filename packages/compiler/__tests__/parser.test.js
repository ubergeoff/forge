...
describe('parseSFC — basic block detection', () => {
    ...
    it('parses a file with all three block types', () => {
        const source = src('<script>', "const x = 1;", '</script>', '<template>', '  <div>hello</div>', '</template>', '<style>', '  div { color: red; }', '</style>');
...
describe('parseSFC — content extraction', () => {
    ...
    it('handles multiline blocks with blank lines', () => {
        const source = src('<script>', '', 'const x = 1;', '', 'const y = 2;', '</script>');
...
describe('parseSFC — multiple style blocks', () => {
    it('collects all style blocks in order', () => {
        const source = src('<style>.a { color: red; }</style>', '<style scoped>.b { color: blue; }</style>');
        const d = parseSFC(source, 'x.forge');
...
describe('parseSFC — error handling', () => {
    it('throws a descriptive error for an unclosed script block', () => {
        expect(() => parseSFC('<script>const x = 1;', 'bad.forge')).toThrow('[Forge Parser]');
        expect(() => parseSFC('<script>const x = 1;', 'bad.forge')).toThrow('bad.forge');
    });
    it('throws for an unclosed template block', () => {
        expect(() => parseSFC('<template><div>', 'bad.forge')).toThrow(/Unclosed <template>/);
    });
    it('throws for an unclosed style block', () => {
        expect(() => parseSFC('<style>.a{}', 'bad.forge')).toThrow(/Unclosed <style>/);
    });
});
describe('parseSFC — real-world fixture', () => {
    it('parses a realistic counter component', () => {
        const source = src('<script lang="ts">', "  import { signal } from '@forge/core';", '  const count = signal(0);', "  function increment() { count.update(n => n + 1); }", '</script>', '', '<template>', '  <div class="counter">', '    <button @click="increment">+</button>', '    <span>{count()}</span>', '  </div>', '</template>', '', '<style scoped>', '  .counter { display: flex; gap: 8px; }', '</style>');
        const d = parseSFC(source, 'counter.forge');
