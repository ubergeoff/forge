...
describe('parseSFC — error handling', () => {
    it('throws a descriptive error for an unclosed script block', () => {
        expect(() => parseSFC('<script>const x = 1;', 'bad.vorra')).toThrow('[Vorra Parser]');
        expect(() => parseSFC('<script>const x = 1;', 'bad.vorra')).toThrow('bad.vorra');
    });
    it('throws for an unclosed template block', () => {
        expect(() => parseSFC('<template><div>', 'bad.vorra')).toThrow(/Unclosed <template>/);
    });
    it('throws for an unclosed style block', () => {
        expect(() => parseSFC('<style>.a{}', 'bad.vorra')).toThrow(/Unclosed <style>/);
    });
});
describe('parseSFC — real-world fixture', () => {
    it('parses a realistic counter component', () => {
        const source = src('<script lang="ts">', "  import { signal } from '@vorra/core';", '  const count = signal(0);', "  function increment() { count.update(n => n + 1); }", '</script>', '', '<template>', '  <div class="counter">', '    <button @click="increment">+</button>', '    <span>{count()}</span>', '  </div>', '</template>', '', '<style scoped>', '  .counter { display: flex; gap: 8px; }', '</style>');
        const d = parseSFC(source, 'counter.vorra');
